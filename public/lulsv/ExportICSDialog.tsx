...
import { LECTION_NAMES, LECTION_NAMES_SHORT, MONTH_LOCALE_SHORT, TIME_TO_RANGE } from "./lu-dati";
import { getLectionKey, getRoomAndProf, isLectionOnDay, type Lection, type PersonData } from "./table-gen";
import ical, { ICalCalendarMethod } from "ical-generator";
import { downloadFile } from "../util";

export const ExportICSDialog = (attribs: Record<string, unknown>) => {
  const calendar = new Calendar(new Date());
  const getData = attribs.getData as () => PersonData | null;
  if (!getData) {
  ...
  const exportButton = <button class="btn btn-primary disabled mb-1 p-centered" style="width: 90%" on:click={() => {
    const cal = ical({
    ...
    for (let i = new Date(rangeStart!); i.getTime() <= rangeEnd?.getTime()!; i.setDate(i.getDate() + 1)) {
      for (const lection of getData()!.lections) {
        if (isLectionOnDay(i, lection)) {
          ...
          const end: [number, number] = TIME_TO_RANGE[lection.time as keyof typeof TIME_TO_RANGE].split(" - ")[1].split(":").map(Number) as [number, number];
          const { room, prof } = getRoomAndProf(getLectionKey(lection)) ?? {};
          cal.createEvent({
            summary: LECTION_NAMES_SHORT[lection.name as keyof typeof LECTION_NAMES_SHORT],
            description: `${lection.name.replace(/[ILP]$/, "")}: ${LECTION_NAMES[lection.name as keyof typeof LECTION_NAMES]}${prof ? `\n${prof}` : ""}`,
  ...
  }}>Izvēlies datumus!</button>;

  calendar.appendInFooter(<div>
    {exportButton}
  </div>);
