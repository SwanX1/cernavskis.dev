...
import { DAY_LOCALE, DAY_LOCALE_LOCATIVE, LECTION_LINKS, LECTION_NAMES, TIME_TO_RANGE } from "./lu-dati";
...
export function createLectionModal(lection: Lection): HTMLElement {
  ...
  const modal: HTMLElement = <div class="modal active">
    <div class="modal-overlay"></div>
    <div class="modal-container">
      <div class="modal-header">
        <button class="btn btn-link float-right" on:click={closeModal}><i class="icon icon-cross"></i></button>
        ...
        <div class="content">
          {
            lection.name === "DatZB067L" ?
              <span class="text-error">Kurss sākas tikai ar 4. nedēļu<br /></span> :
              <></>
          }
                ...
                <td>
                  {
                    ...
                    Array.isArray(lection.weekFilter) ? `Nedēļās ${lection.weekFilter.map(num => `${num}.`).join(", ")}` :
                    "Katru nedēļu"
                  }, {
      ...
      <div class="modal-footer">
        {
          lection.name in LECTION_LINKS ?
            <button class="btn" on:click={() => window.location.assign(LECTION_LINKS[lection.name as keyof typeof LECTION_LINKS])}><i class="icon icon-link"></i> Atvērt e-studijās</button> :
            <></>
        }
      </div>
    </div>
